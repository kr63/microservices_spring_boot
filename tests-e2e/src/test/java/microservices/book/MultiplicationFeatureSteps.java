...
public class MultiplicationFeatureSteps {
    ...
    public void the_user_sends_attempts(final String userAlias,
        ...
        int attemptSent = IntStream.range(0, attempts)
                ...
                .sum();
        assertThat(attemptSent).isEqualTo("right".equals(rightOrWrong) ? attempts : 0)
                .withFailMessage("Error sending attempts to the application");
    }

    @Then("^the user gets a response indicating the attempt is ([^\\s]+$)")
    public void the_user_gets_a_response_indicating_the_attempt_is(final String rightOrWrong) {
    ...
    public void the_does_not_get_any_badge() {
        long userId = lastAttemptResponse.getUser().getId();
    ...
    public void the_user_has_points(final int points) {
        long userId = lastAttemptResponse.getUser().getId();
